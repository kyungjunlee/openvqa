__author__ = 'kyungjunlee'
__version__ = '0.1'

# Interface for accessing the VQA dataset.

# This code is based on the code written by aagrawal for VQA Python API available at 
# https://github.com/MILVLG/openvqa/blob/master/openvqa/datasets/vqa/eval/vqaEval.py
# and Tsung-Yi Lin for MSCOCO Python API available at 
...
class VQA:
	...
	def getImageId(self, image_attr):
		return image_attr.split('.')[0]
	...
	def createIndex(self):
		...
		for each in self.dataset:
			...
			image_id = each['image']
			# same as QA, dict with key as image id and values is all information
			imgToQA[image_id] = each
			# dict with key as image id and value is all information from json file for that id
			qa[image_id] = each
			# dict with the key as image id and value as the actual question
